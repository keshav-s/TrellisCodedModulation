...
% Parameters
LL = 150; % Total number of bits Default is 100
N = 11; % Length of filter in symbol periods. Default is 51
fs = 30; % Over-sampling factor (Sampling frequency/symbol rate). Default is 100
sigma_n = 1; % Noise standard deviation. Default is 1
delay = 1/2; % Offset from optimum sampling point (as fraction of symbol period). 
           % Try either 0 or 1/2 for delay
...
% Create image bits
shannon = imread('USRP demo/images/shannon1440.bmp');
shannon = shannon(:);
bits = shannon(1:150);
